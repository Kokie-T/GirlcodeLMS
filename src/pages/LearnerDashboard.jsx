...
export default function LearnerDashboard() {
  ...
  // --- Fetch learner profile, results, facilitators ---
  useEffect(() => {
    if (!user) return;
    ...
    const fetchData = async () => {
      try {
        ...
        resultSnap.forEach((doc) => {
          ...
          if (!resMap[data.courseId])
            resMap[data.courseId] = { total: 0, count: 0 };
        ...
        const facList = facSnap.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
  ...
  useEffect(() => {
    ...
    const qConv = query(convRef, where("participants", "array-contains", user.uid));

    let unsubMsg;

    const fetchOrCreateConversation = async () => {
      const snap = await getDocs(qConv);
      let convDoc = snap.docs.find((d) =>
      ...
      const msgQ = query(msgRef, orderBy("createdAt", "asc"));
      unsubMsg = onSnapshot(msgQ, (msgSnap) => {
                    ...
                    {selectedFacilitator ? (
                      <>
                        ...
                        <div className="flex mt-2">
                          ...
                          />
                          <button
                            className="ml-1 px-2 bg-blue-500 text-white rounded"
                            onClick={sendMessage}
                          >
                            Send
                          </button>
